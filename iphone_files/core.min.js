...
domain:env.servicesBaseDomain,date:"",proximity:50,includeStores:false},a);a.url=b.assembleUrl(a);if(String(a.format).toLowerCase()==="jsonp"){if(!a.jsonpCallback)a.jsonpCallback="apiGeoLocate";a.jsonpCallback+=d.iReq!==0?"_"+d.iReq:""}if(!a.timeout)a.timeout=env.ajaxTimeout;b.request(a)},reviews:function(a){var b=this,d=b.options;a=c.extend({domain:d.domain,format:d.format,type:"reviews",cache:false},a);a.url=b.assembleUrl(a);if(String(a.format).toLowerCase()==="jsonp"){if(!a.jsonpCallback)a.jsonpCallback=
